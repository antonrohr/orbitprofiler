...
ConnectToStadiaWidget::ConnectToStadiaWidget(QWidget* parent)
  ...
  {
    // Move radioButton from titleBarLayout to mainFrame and position
...
void ConnectToStadiaWidget::SetSshArtifacts(SshConnectionArtifacts* ssh_connection_artifacts) {
  CHECK(ssh_connection_artifacts != nullptr);
  ssh_connection_artifacts_ = ssh_connection_artifacts;
}

void ConnectToStadiaWidget::SetConnection(StadiaConnection stadia_connection) {
  selected_instance_ = std::move(stadia_connection.instance_);
  service_deploy_manager_ = std::move(stadia_connection.service_deploy_manager_);
  grpc_channel_ = std::move(stadia_connection.grpc_channel_);
}
...
void ConnectToStadiaWidget::Start() {
  ...
  if (selected_instance_ != std::nullopt && service_deploy_manager_ != nullptr &&
...
std::optional<StadiaConnection> ConnectToStadiaWidget::StopAndClearConnection() {
  ...
  return StadiaConnection(std::move(selected_instance_.value()), std::move(service_deploy_manager_),
...
void ConnectToStadiaWidget::SetupStateMachine() {
  ...
  s_instances_loading_ = new QState();
  state_machine_.addState(s_instances_loading_);
  ...
  s_connected_ = new QState();
  state_machine_.addState(s_connected_);
  ...
  // STATE s_idle
  s_idle->addTransition(ui_->refreshButton, &QPushButton::clicked, s_instances_loading_);
  ...
  s_instances_loading_->addTransition(this, &ConnectToStadiaWidget::ErrorOccurred, s_idle);
  s_instances_loading_->addTransition(this, &ConnectToStadiaWidget::ReceivedInstances, s_idle);
  ...
  s_deploying->addTransition(ui_->instancesTableOverlay, &OverlayWidget::Cancelled,
                             s_instance_selected);
  s_deploying->addTransition(this, &ConnectToStadiaWidget::Connected, s_connected_);
  ...
  s_connected_->addTransition(ui_->instancesTableOverlay, &OverlayWidget::Cancelled,
                              s_instance_selected);
  s_connected_->addTransition(this, &ConnectToStadiaWidget::ErrorOccurred, s_instance_selected);
}
