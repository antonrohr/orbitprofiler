...
TEST(TargetLabel, Constructor) {
  ...
  EXPECT_FALSE(label.GetIcon().has_value());
}
...
TEST(TargetLabel, ChangeToStadiaTarget) {
  ...
  ASSERT_TRUE(label.GetIcon().has_value());
  EXPECT_EQ(label.GetIcon().value(), TargetLabel::Icon::kProcessAlive);
}
...
TEST(TargetLabel, SetProcessCpuUsage) {
  TargetLabel label({});
  ...
  {
    const bool result = label.SetProcessCpuUsage(20);
    EXPECT_FALSE(result);
  ...
  {
    const bool result = label.SetProcessCpuUsage(20.7);
    EXPECT_TRUE(result);
...
TEST(TargetLabel, SetProcessEnded) {
  ...
  EXPECT_NE(updated_color, initial_color);
  {
    ...
    ASSERT_TRUE(label.GetIcon().has_value());
    EXPECT_EQ(label.GetIcon().value(), TargetLabel::Icon::kProcessEnded);
  }
}
...
TEST(TargetLabel, SetConnectionDead) {
  ...
  const QString error_message = "test error message";
  {
    ...
    EXPECT_NE(label.GetColor(), updated_color);
    ASSERT_TRUE(label.GetIcon().has_value());
    EXPECT_EQ(label.GetIcon().value(), TargetLabel::Icon::kConnectionDead);
  }
}
...
TEST(TargetLabel, Clear) {
  ...
  ASSERT_FALSE(label.GetIcon().has_value());
}
