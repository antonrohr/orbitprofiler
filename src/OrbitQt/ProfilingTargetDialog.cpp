...
void ProfilingTargetDialog::SetupStadiaStates() {
  ...
  QObject::connect(&state_stadia_process_selected_, &QState::exited, this,
                   [this] { ui_->targetLabel->Clear(); });
}
...
void ProfilingTargetDialog::SetupLocalStates() {
  ...
  QObject::connect(&state_local_process_selected_, &QState::exited, this,
                   [this] { ui_->targetLabel->Clear(); });
}
...
void ProfilingTargetDialog::SetupFileStates() {
  ...
  QObject::connect(&state_file_selected_, &QState::exited, this,
                   [this] { ui_->targetLabel->Clear(); });
}
...
void ProfilingTargetDialog::OnProcessListUpdate(
  ...
  QMetaObject::invokeMethod(this, [this, process_list = std::move(process_list)]() {
    ...
    if (ui_->processesTableView->selectionModel()->hasSelection()) {
      const auto it =
          std::find_if(process_list.begin(), process_list.end(), [&](const auto& process) {
            return process.pid() == ui_->processesTableView->selectionModel()
                                        ->currentIndex()
                                        .data(Qt::UserRole)
      ...
      if (it != process_list.end()) {
        ui_->targetLabel->SetProcessCpuUsage(it->cpu_usage());
      }
      return;
    }
